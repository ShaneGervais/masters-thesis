\begin{doublespace}
    Pour illustrer ce phénomène, imaginez-vous 
    sur une rue. Quand une ambulance d’urgence 
    arrive de votre côté gauche, vous 
    remarquerez que le bruit semble plus aigu. 
    Par contre, si elle se dirige vers votre 
    droite, le bruit semble grave. Cet effet est 
...
\begin{doublespace}
    ...
    proportionnel à différence de vitesse 
    ...
    \noindent Cela signifie qu’une impulsion possédant une 
    ...
    que ça. Le délai effectué pour les résultats 
    ...
    avec celle initiale pour mesurer le 
    décalage.
\end{doublespace}

\begin{doublespace}
    \noindent L’expérience consiste en un laser pulsé avec 
    une taille temporelle de A qui subit une 
    ...
    motorisé, tandis que l’autre sert à notre 
    fréquence de référence (fréquence initiale 
    (la source)). Les deux bras sont ensuite 
    recombinés avec un séparateur de faisceau 
    (BS) et analysés à l’aide d’un oscilloscope. 
    ...
    1 en mode continu à 0,63 en mode Nyquist 
    (fréquence égale à la moitié de la fréquence 
    d’échantillonnage). Ensuite, nous appliquons 
...
\begin{doublespace}
    ...
    \noindent Voici le résultat obtenu au lab,
\end{doublespace}
...
\begin{doublespace}
    ...
    l’avons souligné par avant, un pic devrait 
...
\begin{figure}[h]
    ...
    \caption{Agrandissement du spectre à 1MHz de la figure \ref{fig:doppler_res}}
...
\begin{doublespace}
    ...
    par l’effet Doppler est estimé à 8,7535 kHz. Noté 
