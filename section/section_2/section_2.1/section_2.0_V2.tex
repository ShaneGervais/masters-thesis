...
\begin{doublespace}
    ...
    \noindent En photonique quantique, ainsi, pour la caractérisation des états de polarisation, cette matrice densitée peut également être exprimée en termes des paramètres de Stokes \cite{Kwiat}. Les paramètres de Stokes décrivent complètement l’état de polarisation, et ils sont liés aux probabilités de détection dans différentes bases de polarisation \cite{hecht2012optics}. 
    ...
    \noindent Pour un état pur, comme notre exemple, on a la propriété $Tr(\hat{\rho}) = 1$, ce qui se traduit par une cohérence quantique maximale. En revanche, un état mixte se caractérise par une matrice densitée statistique, qui est une somme pondérée d'états purs:
...
\begin{doublespace}
    ...
    \noindent Soit $g$ la constante de couplage, qui doit être réelle pour que le hamiltonien soit hermitien, et $\hat{q}$ la variable pointeur conjuguée de l'observable $\hat{S}$. Cette réduction de l’état quantique se traduit par un déplacement de la position du pointeur, initialement dans l’état $\ket{\xi}_P =\ket{\bar{p} = 0}_P$ dans la base $P$, où $\bar{q}$ représente la valeur moyenne d’une variable $p$ avec une distribution de probabilité $\sigma$. Le pointeur et l’état mesuré sont étroitement liés dans un état décrivant l’ensemble du système $T$, initialement écrit sous la forme :

    \begin{equation}
        \ket{\Psi^i}_T = \ket{\psi}_S \otimes \ket{\bar{p}=0}_P    
    \end{equation}

    \noindent Après la mesure, le pointeur se déplace en fonction de la force d’interaction $\delta\equiv\frac{gt}{\hbar}$ et de la valeur propre $s_j$ de l’observable $\hat{S}$, $\Delta p =\delta s_j$. Ce dernier s’écrit dans lequel l’état du pointeur passe de sa position initiale $\ket{\bar{p} = 0}_P$ à $\ket{\bar{p} =\delta s_j}_P$. Lorsque combiné, l'état du système évolue comme suit :
    ...
    \begin{align}
        ...
        &= \bra{\varphi}_S\ket{\psi}_S\Bigl[ 1 - i\delta \expval{S_W}\hat{p} \Bigr] \ket{\bar{p} = 0} \otimes \ket{\varphi}_S\\
        &= \bra{\varphi}_S\ket{\psi}_S e^{-i\delta \expval{S_W}\hat{q}} \ket{\bar{p} = 0} \otimes \ket{\varphi}_S\\
        &= \ket{\psi}_S e^{-i\delta \expval{S_W}\hat{q}}\ket{\bar{p}=0}
    \end{align}
    ...
    \begin{align}
        \bra{\bar{p} = \delta s_j}\hat{p}\ket{\bar{p}=\delta s_j} &= -i\delta \mathcal{R}\Bigl(\expval{\hat{S}_W}\Bigr)\bra{\bar{p}=\delta s_j}(\hat{p}\hat{q} - \hat{q}\hat{p})\ket{\bar{p}=\delta s_j}\\
        &+ \delta \mathcal{I}\Bigl(\expval{\hat{S}_W}\Bigr)\bra{\bar{p} = \delta s_j}(\hat{p}\hat{q} + \hat{q}\hat{p})\ket{\bar{p}=\delta s_j}\\
        &= \delta\mathcal{R}\Bigl(\expval{\hat{S}_W}\Bigr) = \expval{\hat{p}}
    \end{align}

    \noindent Ainsi pour la variable conjuguée:

    \begin{align}
        \bra{\bar{q} = \delta s_j}\hat{q}\ket{\bar{P}=\delta s_j} &= -i\delta \mathcal{R}\Bigl(\expval{\hat{S}_W}\Bigr)\bra{\bar{p}=\delta s_j}(\hat{q}^2 - \hat{q}^2)\ket{\bar{p}=\delta s_j}\\
        &+ \delta \mathcal{I}\Bigl(\expval{\hat{S}_W}\Bigr)\bra{\bar{p} = \delta s_j}(\hat{q}^2 + \hat{q}^2)\ket{\bar{p}=\delta s_j}\\
