...
\begin{doublespace}
    ...
    permettent de reconstruire les éléments de la matrice de densité 
    ...
    décrivent complètement l’état de polarisation et ils sont liés aux probabilités de détection dans 
    ...
    de visualiser l'état de polarisation sur la sphère de Poincaré. 
...
\begin{doublespace}
    ...
    \noindent où $\mathcal{O}(\delta^2)$ correspond aux ordres plus 
    ...
    \begin{align}
        ...
        &= \bra{\varphi}_S\ket{\psi}_S\Bigl[ 1 - i\delta \expval{\hat{S}}_W \hat{p} \Bigr] \ket{\bar{p} = 0} \otimes \ket{\varphi}_S\\
        &= \bra{\varphi}_S\ket{\psi}_S e^{-i\delta \expval{\hat{S}}_W \hat{q}} \ket{\bar{p} = 0} \otimes \ket{\varphi}_S\\
        &= \ket{\psi}_S e^{-i\delta \expval{\hat{S}}_W \hat{q}}\ket{\bar{p}=0}
    \end{align}
    ...
    \cite{Lundeen_Resch}, ainsi on peut en déduire directement les 
...
\begin{doublespace}
    ...
    dont $\int g(t) dt = \tau$ et se trouve dans le régime de 
...
\begin{doublespace}
    ...
    \end{equation}
     ...
     \begin{equation}
        \expval{\hat{\pi}^{J}}_W = \frac{\bra{D}\hat{\pi}^{J}\ket{\psi^{in}}}{\bra{D}\ket{\psi^{in}}} = \sqrt{N}\bra{J}\ket{\psi^{in}}
    \end{equation}
    ...
    \begin{equation}
        \ket{\psi^{in}} = \frac{1}{\sqrt{N}}\Bigl(\expval{\hat{\pi}^{H}}_W\ket{H}+\expval{\hat{\pi}^{V}}_W\ket{V}\Bigr)
    \end{equation}

    \noindent Puisque $N$ est une constante de normalisation
    ...
    nous pouvons écrire l'état quantique d'entrée en fonction de la
    valeur faible mesurée:

    \begin{equation}
        \ket{\psi^{in}} = \frac{1}{\sqrt{N}}\Bigl(\expval{\hat{\pi}^{H}}_W\ket{H}+ \Bigl(1- \expval{\hat{\pi}^{H}}_W\Bigr)\ket{V}\Bigr)
    \end{equation}
    ...
    de la partie imaginaire \cite{Hairiri}. Et, à partir des données expérimentales des deux 
    valeurs moyennes $\expval{\hat{t}}$ et $\expval{\hat{\omega}}$, nous 
    pouvons calculer directement les amplitudes de probabilité. 

    \begin{align}
        |a| &= \sqrt{\frac{\mathcal{R}(\expval{\hat{\pi}^{H}}_W)}{\tau}}\label{eq:amplitude_a}\\
