...
\begin{onehalfspace}
    ...
    d’entrée est préparé à l'aide des lames d'ondes et ensuite 
    soumis à une mesure faible en 
    introduisant un court délai de $167$ $ps$ avec une différence de parcours 
    de $5$ $cm$ entre 
    les deux composantes $H$ et $V$. 
...
\begin{figure}[!htpb]
    ...
    et un PBS qui divise les états de polarisation 
    ...
    réalisée par une lame demi-onde inclinée à $45$ degrés 
    par rapport à un polariseur. 
    Cela crée une projection sur l’état $\ket{D}$ qui permet d'extraire le 
    chevauchement des états $\ket{H}$ et $\ket{V}$.  
    Ensuite, le signal expérimental est détecté avec un
    photodétecteur et acquis par l'oscilloscope.
}
    \label{fig:realexp}
\end{figure}
...
\begin{onehalfspace}
    ...
    Les trajets de polarisation sont obtenus en
    modifiant les angles des lames d'onde lors de 
    l'étape de préparation. Le 
    premier trajet correspond à l'ensemble des états de polarisation 
...
\begin{onehalfspace}
    \noindent \textcolor{red}{où l'indice $HWP$ fait référence à une lame demi-onde
    (\guillemetleft half wave plate \guillemetright en anglais) 
    et $\theta$ l'angle de rotation de la lame par 
    ...
    Cette matrice est une matrice de Jones qui 
    ...
    %ce premier trajet réalise une rotation autour de l'axe $S_3$ de la sphère de Poincaré,
    %c'est-à-dire que l'état de polarisation évolue dans la manière suivante:
    %$\ket{H} \to \ket{D} \to \ket{V} \to \ket{A} \to \ket{H}$. 
    \noindent L'état initial est l’état horizontal 
    $\ket{H} \equiv \begin{pmatrix}
        1\\
        0
    \end{pmatrix}$ correspondant dans notre cas à l'état de polarisation 
    transmit par un séparateur 
    de faisceau polarisant. Ensuite, l'état évolue de la façon 
    suivante en fonction de l'angle $\theta$ de la lame d'onde:
\end{onehalfspace}
...
\begin{onehalfspace}
    \noindent Donc, \textcolor{red}{les états d'entrée selon l'angle $\theta$ préparés sont}:
\end{onehalfspace}
...
\begin{onehalfspace}
    ...
    polarisation, nous utilisons la sphère de Poincaré. Les amplitudes de probabilité
    de la fonction d'onde de l'équation \ref{eq:state1} correspondent à la 
    représentation de Jones de la polarisation de l'équation \ref{eq:state1_Jones}. Et, il est possible de 
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=1.0\textwidth]{poincare_sphere_HDVA.png}
    \caption{Schéma du trajet $\ket{H}\rightarrow\ket{D}\rightarrow\ket{V}\rightarrow\ket{A}\rightarrow\ket{H}$
    utilisant seulement une lame demi-onde dans la préparation de l'état initial.
    Ici, un état de polarisation horizontal vecteur $\begin{pmatrix}
        1\\
        0
    \end{pmatrix}$ 
    en représentation de Jones est représenté par le vecteur 
    $\begin{pmatrix}
        1\\
        0\\
        0
    \end{pmatrix}$ en représentation de Stokes sur la sphère de Poincaré. 
    Un état de polarisation 
...
\begin{onehalfspace}
    \noindent Ce premier trajet, correspond à une rotation autour de $S_3$,
    est réalisé en tournant uniquement la lame 
    demi-onde, c'est-à-dire 
    que nous traversons les états de polarisation
    $\ket{H} \to \ket{D} \to \ket{V} \to \ket{A} \to \ket{H}$.
    Dans l'expérience, on tourne 
    l'angle de la lame \textcolor{red}{demi-onde} de $2,5$
    ...
    $\ket{H}$. L’opération de \textcolor{red}{la lame quart d'onde} se définit par 
    la matrice suivante:
\end{onehalfspace}

\begin{align}
    T_{QWP}(\phi) &= e^{\frac{-i\pi}{4}}\begin{pmatrix}
        cos^{2}(\phi) + isin^{2}(\phi) & (1-i)cos(\phi)sin(\phi)\\
        (1-i)cos(\phi)sin(\phi) & sin^{2}(\phi) + icos^{2}(\phi)
    \end{pmatrix}\\
    T_{QWP}(\phi = 0^{\degree}) &= e^{\frac{-i\pi}{4}}\begin{pmatrix}
        1 & 0 \\
        0 & i
    \end{pmatrix}
\end{align}
...
\begin{onehalfspace}
    \noindent où l'indice $QWP$ fait référence 
    à une lame quart d'onde (\guillemetleft quarter wave plate \guillemetright en anglais).
    L'angle $\phi$ est l'angle de rotation de la lame 
    quart d'onde par rapport à l'axe rapide.
    %Cette matrice est une matrice de Jones qui représente la transformation 
    ...
    %simplicité. 
    Avec cette lame d'onde et celle demi-onde, l'état évolue comme suit:
\end{onehalfspace}
...
\begin{equation}
    \ket{\psi_{i}^{2}} = e^{\frac{-i\pi}{4}}cos(2\theta)\ket{H} + e^{\frac{-i\pi}{4}}isin(2\theta)\ket{V}
    \label{eq:state2}
\end{equation}

\begin{onehalfspace}
    \noindent Nous pouvons trouver les paramètres de Stokes pour démontrer sa 
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=1.0\textwidth]{poincare_sphere_HRVL.png}
    ...
    $0$ degré dans la préparation de l'état initial.}
...
\begin{onehalfspace}
    ...
    $\ket{R} \to \ket{A} \to \ket{L} \to \ket{D} \to \ket{R}$. 
    Cette dernière est 
...
\begin{equation}
    T_{QWP}(\phi = 45^{\degree}) = \frac{e^{\frac{-i\pi}{4}}}{2}\begin{pmatrix}
        1-i & 1+i \\
        1+i & 1-i
    \end{pmatrix}
\end{equation}
...
\begin{align}
    ...
    \end{pmatrix} \begin{pmatrix}
        1\\0
    \end{pmatrix}\\
    &= \frac{e^{\frac{-i\pi}{4}}}{2}\begin{pmatrix}
...
\begin{equation}
    \ket{\psi_{i}^{3}} = \frac{e^{\frac{-i\pi}{4}}}{2}\Bigl(((1-i)sin(2\theta) + (1+i)cos(2\theta))\ket{H} + ((1+i)sin(2\theta) + (1-i)cos(2\theta))\ket{V}\Bigr)
    \label{eq:state3}
\end{equation}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=1.0\textwidth]{poincare_sphere_DRAL.png}
    \caption{Schéma du trajet $\ket{R}\rightarrow\ket{A}\rightarrow\ket{L}\rightarrow\ket{D}\rightarrow\ket{R}$
    utilisant une lame demi-onde et une lame quart d'onde à 
    $45$ dégrés dans la préparation de l'état initial}
