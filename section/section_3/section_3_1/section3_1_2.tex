...
\begin{figure}[!h!p!t!b]
    ...
    impulsion laser ultra-courte \cite{ThorlabsNPL64B}, mesurée avec un 
    photodétecteur à base de Si \cite{ThorlabsDET025A} et acquise 
...
\begin{onehalfspace}
    \noindent \textcolor{red}{\sout{Le profil temporel des impulsions n’est 
    ...
    mesure \cite{ThorlabsDET025A}}}. \textcolor{red}{Vue que le profile temporel des impulsion 
    semble à une fonction porte \cite{ThorlabsDET025A} (voir la figure \ref{fig:typical-pulse}), on pourrait ajuster une fonction porte sur
    le signal mais cela créerait une ambiguïté lors de la détermination du temps 
    d'arrivée du signal}. Donc, pour préciser la position temporelle de l'impulsion, nous allons
    effectuer la dérivée de la fonction d'amplitude du signal pour identifier
    le temps d'arrivée du signal \textcolor{red}{via le front montant du signal} représenté par la position temporelle du pic maximal
    ...
    \begin{figure}[!hptb]
        ...
        \caption{Donneés de la dérivée des impulsions de chaque longueur de câble
            BNC RG-58, avec les ajustements paramétriques  
            de type \textit{poly4} représentés par les lignes pointillées.
            La position temporelle du pic de la dérivée de l'impulsion
            est utilisée pour déterminer le temps d'arrivée du signal.
            }
        \label{fig:BNC-pulse}
    \end{figure}
    ...
    \noindent où $a_i$, $b_i$ \textbf{\textcolor{red}{et}} $\omega$ sont les coefficients de la série de 
    Fourier et $t$ est le temps. \textcolor{red}{Notez que $\omega$ est la fréquence fondamentale du
    signal que le fit cherche à modéliser, cette valeur est donc déterminée par le fit lui-même
    et n'est pas un paramètre fixe varient selon les données.}
    Et une fonction gaussienne, dénommée par \textit{gauss1} qui est:
    ...
    Ensuite, nous avons utilisé la fonction \textit{polyval} pour évaluer
    ...
    \textcolor{red}{Le tableau \ref{table:BNC-fits} démontre les temps d'arrivée et
    l'écart-type pour les différents ajustements que nous avons essayés;
    dont le temps d'arrivée est calculé à partir de la position moyenne
    du pic sur $10$ différentes prises de données de la même longueur de câble
    et l'écart-type est la variation de ses positions temporelles
    pour chaque prise de données.
    Nous avons ajouté plus de chiffres significatifs pour les
...
\begin{longtable}{p{2.0cm} p{2.0cm} p{4.0cm} p{2.0cm}}
    ...
    ajustements de courbe pour l'expérience de la vitesse d'un signal dans un câble 
...
\begin{onehalfspace}
    ...
    comme référence, qui est la vitesse d'un signal électrique dans un câble BNC
    \cite{arrl2019,thorlabs2021}.
    

    \begin{longtable}{p{2.0cm} p{4.0cm} p{2.0cm} }
        \caption{Mesure de la vitesse d'un signal électrique dans les câbles BNC 
        ...
        % Paste all your rows here like:
        poly2     & 197394356     & 0.2369 \\
        poly3     & 197387263     & 0.2405 \\
        poly4     & 197394687     & 0.2367 \\
        poly5     & 197450416     & 0.2085 \\
        poly6     & 201496092     & 1.8362 \\
        poly7     & 203679907     & 2.9399 \\
        poly8     & 197533854     & 0.1664 \\
        poly9     & 197529188     & 0.1687 \\
        fourier1  & 197389806     & 0.2392 \\
        fourier2  & 197386245     & 0.2410 \\
        fourier3  & 196958823     & 0.4570 \\
        fourier4  & 197340398     & 0.2641 \\
        fourier5  & 198306506     & 0.2241 \\
        fourier6  & 197820685     & 0.0214 \\
        gauss1    & 197388576     & 0.2398
    ...
    une forme plus simple. Cet ajustement de
    ...
    \noindent où $a_0$, $b_0$, $c_0$ et $d_0$ sont les paramètres d’ajustement
    du polynôme, $y$ représente la dérivée de nos données et 
    $t$ représente le temps. La figure \ref{fig:BNC-res} 
    ...
    \begin{figure}[!hptb]
        \centering
        \includegraphics[width=1.0\textwidth]{speed_BNC copy 5.png}
        \caption{
        Détermination de la vitesse d'un signal électrique dans un câble BNC
        RG-58 où $\tau$ est le temps de propagation et $\Delta L$ est 
        la variation des longueurs de câble. Les points représentent les délais mesurés pour chaque
        longueur de câble BNC, tandis que la ligne représente l'ajustement
        linéaire des données.
        Les barres d’erreur horizontales représentent l’incertitude de nos
        câbles BNC, environ $\pm 2\%$ par le manufacturier. Les barres
    ...
    L'ajustement linéaire des données nous donne une pente de
    $0,00506615$ $ns/mm$ qui correspond à $\frac{3}{2c}$ (à rafiner en fonction de la formule) et un coefficient de corrélation
    $R^2$ de $0,9999$,
    ce qui indique que les données sont bien ajustées par une droite.
    Notre résultat pour la vitesse d'un signal électrique dans un câble BNC est
    $197 388 575 \pm 0,0008$ $m/s$, ce qui représente une erreur en
    pourcentage de $0,2367\%$ par rapport à la vitesse du
    signal dans un câble BNC qui est environ $0,66$ par
    rapport à $c$ selon \cite{arrl2019} (voir le tableau \ref{table:speed-BNC-table}) ce qui est
