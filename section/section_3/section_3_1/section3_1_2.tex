\begin{doublespace}
    ...
    (temps de équivalent) et acquérons des données sur une durée 
    de $100$ $ns$, avec une longueur d'enregistrement de $10 000$ points
    , une résolution temporelle de $4$ $ps$, moyenné sur $10 000$ formes d'ondes
    pour $10$ différentes prise de données. La figure \ref{fig:typical-pulse}
...
\begin{doublespace}
    ...
    ressemble de plus en plus à une fonction de porte au fur et à 
    mesure. Pour préciser la position temporelle de l'impulsion, nous allons
    effecteur la dérivée de la fonction d'amplitude du signal pour identifié
    le temps d'arrivée du signal représenté par le pic maximale 
    de la dérivée. Ensuite, je vais ajuster une courbe à ce pic pour
    déterminer la position temporelle du pic (le temps d'arrivée du signal) 
    avec une précision maximale. Nous allons utiliser MATLAB pour
    effectuer ces ajustements. J'ai essayé plusieurs types
    ...
    \noindent Les ajustements de la courbe devient un 
    aspect vraiment important dans la localisation du temps d’arrivée 
    de notre signal. Je effectue de nombreux ajustements de courbe 
    pour chaque longueur de câble, en utilisant les données d’amplitude
    initiales de l’impulsion. Pour les ajustements j'utiliser une 
    résultion de $1/10000$ de la résolution de l’oscilloscope. Ainsi 
    qu’en négligeant environ $40 \%$ des données 
    ...
    tableau des temps d’arrivée, puisque les comparent au temps d'arrivée
    du premier câble, qui est considéré comme notre origine.
...
\begin{doublespace}
    ...
    présente les différennts type d'adjustement, j'utilise également 
    ...
    pour évaluer la qualité de l'ajustement. Je procède l’ajustement 
    ...
    déterminée en effectuant un ajustement linéaire des les valeurs 
    ...
    \noindent Il inclut la vitesse mesurée, la vitesse 
    ...
    erreurs en pourcentage supérieures à $0,7\%$. Je conclus donc, 
    selon les deux tableaux, le 
    ...
    ajustement gaussien. Je choisi un ajustement polynôme 
    ...
    fichiers et elle est optimal pour obtenir la 
    meilleure résolution possible pour les procédures directes via 
    mesure faible, ainsi que la forme typique qu’on retrouve. 
