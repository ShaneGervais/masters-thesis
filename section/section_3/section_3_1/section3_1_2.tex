...
\begin{onehalfspace}
    \noindent Vue que le profile temporel des impulsion 
    ressemble à une fonction porte \cite{ThorlabsDET025A} (voir la figure \ref{fig:typical-pulse}), on pourrait ajuster une fonction porte sur
    ...
    le temps d'arrivée du signal via le front montant du signal représenté par la position temporelle du pic maximal
    ...
    \noindent où $a_i$, $b_i$ et $\omega$ sont les coefficients de la série de 
    Fourier et $t$ est le temps. Notez que $\omega$ est la fréquence fondamentale du
    signal que le fit cherche à approximer, cette valeur est donc déterminée par le fit lui-même
    ...
    Le tableau \ref{table:BNC-fits} démontre les temps d'arrivée et
