\begin{doublespace}
    ...
    Les signaux sont détecté par les photodétecteurs rapides, 
    voir la figure \ref{fig:speed-of-light}, une partie du faisceau 
    laser est détournée à l’aide d’un PBS 
    vers un photodétecteur rapide, dont la sortie sert de 
    \guillemetleft trigger \guillemetright (déclencheur) 
    pour l'oscilloscope (signal de référence). Le signal expérimental, 
    détecté est ensuite acquis en tant que voie principale sur 
    l’oscilloscope afin de mesurer le délai temporel par rapport à 
    cette référence. Je utilise le port externe de l’oscilloscope 
    ...
    notre référence, qui correspond à longueur initial du câble
    ...
    de pic, enveloppe, haute résolution et moyennage. Je choisi 
    le mode d'acquisition moyen, car il me fournit le signal de 
    sortie le plus typique du laser. Pour obtenir un signal de sortie
    représentatif de l'impulsion du laser, 
    je dois effectuer un moyennage sur $10 000$ formes d'onde expérimentaux. 
    ...
    l’interpolation et temps d'équivalent. En mode 
    ...
    ’échantillonnage par temps d'équivalent permet d’augmenter 
    ...
    \begin{figure}[!hptb]
        ...
        d’échantillonnage en temps de équivalent de l’oscilloscope 
        ...
        L’oscilloscope fonctionne en mode de temps d'équivalent
    ...
    je acquiert un petit nombre d’échantillons au moment
    ...
    sortie au format CSV. Une mesure d'acquisition prend environ trois minutes et
    je prend $10$ différentes acquisitions pour chaque different délai. 
