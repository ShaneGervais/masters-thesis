\begin{onehalfspace}
    ...
    référence et un signal expérimental. Nous obtenons \textcolor{red}{le signal expérimental}
    ...
    Le signal de référence \textcolor{red}{déclenche l'oscilloscope} sur son front montant et 
    ...
    de pic, enveloppe, haute résolution et moyen. Nous choisissons
    ...
    \textcolor{red}{ce qui réduit le bruit de fond d'un facteur $\sqrt{1000} \approx 32$}.
    ...
    interpolation et en temps équivalent
    ...
    temporelle est insufisante pour nos mesures. Le mode d’échantillonnage en temps équivalent permet d’augmenter 
    numériquement le taux d’échantillonnage au-delà du taux d’échantillonnage 
    ...
    \begin{figure}[!hptb]
        ...
        répétitif en cours d’acquisition \textcolor{red}{pour reconstruire le signal complet}. 
        Il régule ensuite la
    ...
    sortie \textcolor{red}{\sout{au format \textsc{CSV}}}. Une mesure d'acquisition 
    ...
    câble \textcolor{red}{BNC RG-58 $50 \Omega$} \cite{ThorlabsBNC} avec des délais temporels implémentés
    en faisant parcourir le signal expérimental à travers des câbles de
    différentes longueurs. Cette expérience 
    ...
    $172$, $270$, $522$, $1032$ et $3000$ $mm$ 
    (avec $\pm 2 \%$ d'erreur de fabrication \cite{ThorlabsBNC}). 
    Les délais 
    commencent par une mesure \textcolor{red}{avec le câble le plus courte}; dans ce cas, le miroir ajustable est fixe. Pour 
    l'expérience de mesure de la vitesse de la lumière, nous introduisons
    un délai en déplaçant le miroir à différentes distances $\Delta x$: 
    $2,52$-$5,48$ $cm$ $\pm 2$ $\mu m$ (converti à partir des pouces, car 
    notre platine de translation linéaire manuelle utilise des unités 
    impériales \cite{EdmundOpticsStage}) et 
    $10,10$-$20,19$-$30,29$-$40,38$-$50,48$ et $65,62$ $cm$ $\pm 2$ $mm$.
    Les deux premières distances sont mesurées avec une platine de 
    translation linéaire manuelle, tandis que les autres sont mesurées
    avec une règle d’un mètre de longueur. La platine est à $0,00$ $cm$
    lorsque le miroir est à la position initiale, les autres distances sont
    mesurées par rapport à cette position. L'incertitude des distances mesurées
    avec la règle est de $2$ $mm$, car nous prenons en compte le
    positionnement de la règle à la position initiale et à la position à mesurer. 
    Le miroir est déplacé
    manuellement à chaque fois que nous voulons mesurer un nouveau délai.
    Comme le montre
