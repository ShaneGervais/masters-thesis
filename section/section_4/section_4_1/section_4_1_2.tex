...
\begin{onehalfspace}
    ...
    comme nous l'avons vu dans les sous-sections précédentes. Ainsi, les
    ...
    d'entrée. Nous \textcolor{red}{attribuons} ce décalage au terme additionnel
    mentionné précédemment, notamment le terme le terme 
    $2sin(\theta)cos(\theta)$ qui est présent dans l'équation \ref{eq:expval_t_1}.
    Ce terme additionnel apporte des valeurs négatives pour la partie réelle de la valeur faible
    pour certains états d'entrée à ce trajet de polarisation.
    ...
    longueur de cohérence \textcolor{red}{car la largeur des pics dans la visibilité est très courte ainsi
    que l'espacement entre les pics est large démontrant une faible longueur de cohérence} (voir figure \ref{fig:vis}).
    Nous démontrons
    ...
    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=1.0\textwidth]{real_part_visibility_HDVA.png}
        \caption{Effet de la visibilité de l'interférence sur la partie réelle de la valeur faible
        pour le trajet de polarisation linéaire.
        Les valeurs démontrées sont $\mathcal{V} = 1, 0.5, 0.1, 0.01$.}
        \label{fig:vis_real}
    \end{figure}

    \noindent Ainsi, ce 
    terme \textcolor{red}{\sout{génère du bruit et}} provoque le décalage observé entre les équations
    ...
    figure \ref{fig:vis_real_path_1}.

    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=1.0\textwidth]{real_weak_value_vis_3_analysis.png}
        \caption{Trajet de polarisation $\ket{H} \to \ket{D} \to \ket{V} \to \ket{A} \to \ket{H}$
        avec ajustement paramétrique pour le terme d'interférence. La valeur de visibilité
        obtenue est de $\mathcal{V} = 0.11$.}
        \label{fig:vis_real_path_1}
    \end{figure}

    \noindent Nous calculons que la visibilité du terme additionelle $2sin(\theta)cos(\theta)$ 
    est de $\mathcal{V} = 0.11$, \textcolor{red}{correspondant bien à l'interaction faible de $167$ $ps$ 
    équivalent à un déplacement du miroir de $2.5$ $cm$ ($5$ $cm$ aller-retour) correspondant à 
    la valeur du pic de visibilité à cette position (voir la figure \ref{fig:vis})}, ce qui démontre que ce terme peut 
    déplacer la partie réelle de la valeur faible. \textcolor{red}{MIS UN ESPACE POUR SÉPARÉ LE PARAGRAPHE}
    
    \noindent Ce décalage est d'environ $11$ degrés du centre.
    Cette valeur de visibilité de $0.11$ provient d’un ajustement de l'équation
    \ref{eq:vis_eq} aux données expérimentales. En optimisant le paramètre 
    $\mathcal{V}$, nous avons trouvé que cette valeur reproduisait le mieux le 
    ...
    \noindent Pour le trajet de polarisation en superposition, nous pouvons 
    aussi \textcolor{red}{déterminer} l'effet du terme d'interférence en fonction de la visibilité 
    avec l'équation:

    \begin{equation}
        \expval{\hat{t}}_{vis}^{superposition} = \frac{\tau}{2}(1 + \mathcal{V}\frac{sin(2\theta)}{4})\label{eq:vis_eq_superposition}
    \end{equation}

    \noindent Nous montrons des différentes valeurs de visibilité et comment le terme d'interférence
    affecte la partie réelle de la valeur faible de ce trajet de polarisation
    dans la figure \ref{fig:vis_real_DRAL}.

    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=1.0\textwidth]{real_part_visibility_DRAL.png}
        \caption{Effet de la visibilité de l'interférence sur la partie réelle de la valeur faible
        pour le trajet de polarisation en superposition. Les valeurs démontrées sont
        $\mathcal{V} = 1, 0.5, 0.1, 0.01$.}
        \label{fig:vis_real_DRAL}
    \end{figure}

    \noindent Nous allons maintenant examiner les valeurs des visibilités
    nécessaires pour les trajets de polarisation linéaire avec un
    ajustement paramétrique avec l'équation \ref{eq:vis_eq_superposition} dans la
    figure \ref{fig:vis_real_path_3}.

    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=1.0\textwidth]{real_part_vis_5_analysis copy.png}
        \caption{Trajet de polarisation $\ket{R} \to \ket{A} \to \ket{L} \to \ket{D} \to \ket{H}$
        avec ajustement paramétrique pour le terme d'interférence. La valeur de visibilité
        obtenue est de $\mathcal{V} = 0.09$.}
        \label{fig:vis_real_path_3}
    \end{figure}

    \noindent Nous avons observé une visibilité similaire à celle du trajet de polarisation linéaire
    avec une valeur de $\mathcal{V} \approx 0.09$ pour le trajet de polarisation en superposition,
    démontrant que le terme d'interférence est cohérent avec ces trajets de polarisation. 
    Encore, cette valeur de visibilité de 
    $0.09$ provient d’un ajustement de l'équation
    \ref{eq:vis_eq_superposition} aux données expérimentales. Le terme d'interférence
    explique la nature oscillatoire de nos données expérimentales
    pour le trajet de polarisation en superposition. Ceci se voit dans la figure \ref{fig:vis_real_path_3} et correspond aux 
    incertitudes expérimentales observées. Nous constatons que du fait de la faible longueur de cohérence du laser,
    les effets des termes d'interférences apparaissant dans la formule de la
    valeur faible ne sont pas aussi importants que prévu. Cependant, en prenant
    en compte la visibilité du laser, nous sommes capables d'expliquer le
    comportement des courbes expérimentales à l'aide des formules théoriques
    revisitées.
