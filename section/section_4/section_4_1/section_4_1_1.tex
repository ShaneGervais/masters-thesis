\begin{doublespace}
    ...
    calibration pour obtenir le délai associé à cet état. Notons que nous 
    redéfinissons l’angle $\theta \equiv 2\theta$ pour qu’il ait une 
    forme similaire à 
    un plan circulaire et pour négligé le facteur de $2$. Nous combinons 
    les résultats pour la partie 
    réelle de la valeur faible correspondant à nos valeurs théoriques 
    calculées dans le chapitre précédent.
    Voici les 
    résultats de chaque délai pour les différents états d’entrée pour 
    le trajet 
    $\ket{H}\rightarrow\ket{D}\rightarrow\ket{V}\rightarrow\ket{A}$ 
...
\begin{figure}[!h!t!p!b]
    ...
    stade motorisé. Ils sont pas visibles, car ils sont
    ...
    et les barres d’erreurs verticales, l’erreur de la position de 
    l’angle induite par notre stade motorisé.
    }
...
\begin{doublespace}
    ...
    mesurons les délais de chaque état $\psi(\theta)$. En réalité, la courbe 
    théorique de cette partie réelle est une valeur cosinus carrée en 
    ignorant la partie $2sin(\theta)cos(\theta)$ de notre résultat 
    théorique, car elle correspond mieux à nos résultats expérimentaux. 
    Cette partie $2sin(\theta)cos(\theta)$ n’a pas de signification 
    physique, car lorsque nous passons de $\ket{V}$ à $\ket{H}$, nous 
    devrions éviter tout délai négatif, ce que nous constatons 
    effectivement pour garder la symmétrie positive de la mesure. 
    ...
    trouve dans le trajet à venir. Voici les résultats pour le trajet 
...
\begin{doublespace}
    ...
    et $\ket{A}$, mais avec une composante imaginaire. On ne peut pas 
    ...
    le passage de phase ou d’ellipticité des états de polarisation se 
    trouvent dans la partie imaginaire de la valeur faible. Ensuite, 
...
\begin{doublespace}
    ...
    réelle de la faible valeur. Cette constatation met en évidence la 
    symétrie quantique, qui correspond à la symétrie optique des états 
    de polarisation D, A, L et R. Par conséquent, l’ensemble des données 
    ...
    faible. En outre, à partir de ces données pour chaque trajet, nous 
    pouvons directement calculer les amplitudes de probabilité de l’état 
    quantique à partir de ces mesures de délai, comme nous l’avons 
    mentionné dans la section 2.3. 
    Voici $\ket{H} \to \ket{D} \to \ket{V} \to \ket{A}$ 
...
\begin{figure}[!h!t!p!b]
    ...
    de polarisation 
    $\ket{H} \rightarrow \ket{D} \rightarrow \ket{V} \rightarrow \ket{A}$. 
    Les courbes théoriques provenant de nos calculs dans la section 2.3. 
    Les barres d'erreur sont les incertitudes proviennent à partir de notre 
    partie réelle de la valeur faible via Monte-Carlo, car la propagation 
    analytique fait exploser certaines incertitudes de façon irrégulières. }
