...
\begin{figure}[!h!t!p!b]
    ...
    La courbe théorique est une fonction cosinus carrée décalée de
    $11\degree$ de sorte: $cos^2(\theta - 11\degree)$.
...
\begin{doublespace}
    ...
    additionelle de $2sin(\theta)cos(\theta)$, plus sur laquelle nous allons
...
\begin{figure}[!h!t!p!b]
    ...
    faible du trajet de polarisation 
    $\ket{H} \rightarrow \ket{R} \rightarrow \ket{V} \rightarrow \ket{L} \rightarrow\ket{H}$.
    La courbe théorique est une fonction cosinus carrée: $cos^2(\theta)$.}
...
\begin{doublespace}
    ...
    théoriques trouvées dans l'équation \ref{eq:expval_t_2}. 
    On ne peut pas 
    ...
    circulaires, null pour les états
    ...
    Ensuite, voici les résultats pour le trajet 
    ...
    $\ket{D}$ et en terminant avec l'état $\ket{D}$, qui sont
    des états de polarisation linéaires possédant les états de base
    $\ket{H}$ et $\ket{V}$ comme $\ket{R}$ et $\ket{L}$,
    mais avec pas de composante imaginaire
    (figure \ref{fig:HDAV_delay_results}).
\end{doublespace}
...
\begin{doublespace}
    \noindent Cela s’avère captivant, car elle démontre 
    ...
    nos données expérimentales sont conformes se conforment avec une relation
    constante de $\tau/2$ pour la partie réelle de la valeur faible.
    %ce qui est conforme à nos attentes théoriques.
...
\begin{figure}[!h!t!p!b]
    ...
    de l'équation \ref{eq:expval_t_1} soit
    $\sqrt{|cos^2(\theta - 11\degree)|}$ pour $|a|$
    et $\sqrt{|sin^2(\theta - 11\degree)|}$ pour $|b|$.}
...
\begin{doublespace}
    ...
    Les amplitudes de probabilité sont bien représentées par ceux 
    ...
    calculées pour ce trajet de polarisation: \ref{eq:state1} dont 
...
\begin{figure}[!h!t!p!b]
    ...
    par les équations \ref{eq:amplitude_a} et \ref{eq:amplitude_b}.
    Les courbes théoriques provenant de les amplitudes de probabilités
    de l'équation \ref{eq:expval_t_2} soit $\sqrt{|cos^2(\theta)|}$ pour $a$
...
\begin{doublespace}
    ...
    Ces résultats sont également conformes à nos attentes théoriques,
    car les amplitudes de probabilité sont bien représentées par ceux 
    ...
    et $\sqrt{|sin^2(\theta)|}$ pour $b$.
    Nous avons ensuite mesuré les amplitudes de probabilité pour le trajet 
    $\ket{D} \rightarrow \ket{R} \rightarrow \ket{A} \rightarrow \ket{L}$ 
...
\begin{doublespace}
    ...
    soit $\mathcal{R}(a)$ est constante a une valeur de $\frac{1}{\sqrt{2}}$, 
