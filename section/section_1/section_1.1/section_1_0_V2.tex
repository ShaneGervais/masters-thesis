...
\begin{onehalfspace}
    ...
    \textcolor{red}{Contrairement aux ordinateurs classiques, un ordinateur quantique 
    repose sur des bits quantisés appelés qubits, soit $\ket{0}$ et $\ket{1}$,
    qui forment un ensemble binaire de deux ou plusieurs états physiques possibles du système utilisé, comme le spin d'une particule $\ket{\uparrow}$ et $\ket{\downarrow}$ ou
    la polarisation d'un photon $\ket{H}$ et $\ket{V}$ par exemple.} Les lois de 
    ...
    peut contenir simultanément $2^n$ états possibles, où $n$ est le nombre de qubits.
    Cette capacité de superposition permet aux ordinateurs quantiques de
    traiter des informations de manière exponentiellement plus efficace que les ordinateurs classiques.
    
    \noindent Cette utilisation 
    ...
    quantique le perturbe, ce qui rend impossible de copier un état
    quantique contenant un message privé (théorème de non-clonage \cite{Wootters1982}). Des travaux sont
    ...
    Dans le même
    domaine, les gyroscopes quantiques peuvent être utilisés pour des
    ...
    mentionné \cite{QTintelecomindustry,metrology,QTapplImpl,Huang:22,SpinHallEffect,Wang_2019}.
    ...
    \textcolor{red}{En effet, il est impossible de mesurer 
    simultanément deux variables complémentaires (ex: le moment d’arrivée 
    et la fréquence spectrale d'une particule) avec une précision absolue décrit par le 
    ...
    quantique cause une perturbation qui fait s'effondrer \textcolor{red}{l’état dans 
    l’un de ses états propres possibles. Une fois que le système est 
    effondré dans une de ces valeurs propres, des mesures ultérieures
    sur le système donneront toujours le même résultat. La figure 
    \ref{fig:dice} ci-dessous illustre le concept de superposition 
    quantique et son effondrement lors d'une mesure. Une fois que 
    le dé est lancé et que le résultat est observé, le système 
    démeure dans cet état spécifique pour les mesures ultérieures
    (par exemple, l’état $\ket{5}$). Donc, pour obtenir d'autre 
    informations sur l'état initial du système, il est nécessaire 
    d'effectuer plusieurs mesures sur des copies identiques du système
    et reconstruire l'état quantique à partir des statistiques
    obtenues.}
    ...
    \begin{figure}[!htbp]
        ...
        \textcolor{red}{probabilité de $|c_5|^2$} de se produire. Une fois qu’il s’est 
    ...
    \noindent \textcolor{red}{C'est ici que les techniques conventionnelles, comme la tomographie 
    quantique, permettent une reconstruction complète des états à l’aide 
    de plusieurs mesures projectives prédéterminées. Soit que mesurer 
    l’état de polarisation d’un photon, par exemple, nécessite de projeter 
    l’état du photon sur plusieurs bases de mesure spécifiques et d’effectuer 
    ces mesures sur des copies identiques du système pour recueillir
    suffisamment de statistiques pour une reconstruction complète de l’état.
    Toutefois, elles deviennent rapidement inadaptées aux systèmes de grande dimension en
    ...
    \cite{vonNeumann} \textcolor{red}{et exploite un pointeur couplé au système, dont le 
    déplacement minimal appliqué sur le système est proportionnel à une 
    observable complexe nommée la \guillemetleft valeur faible \guillemetright. 
...
\begin{onehalfspace}
    ...
    \textcolor{red}{leur potentiel, notamment en ce qui concerne: la théorie des mesures quantiques, 
    l’informatique quantique, la télécommunication optique et la métrologie quantique
    \cite{Huang:22,SpinHallEffect,Lundeen_Resch,QED,OpticalNetworks,Lundeen_thesis,Brunner_2004}. 
    ...
    \textcolor{red}{à partir d’un décalage fréquentiel \cite{JohnCHowellFrequencyCombs,Salazar,OpticalNetworks,Steinberg_prob_div}. }
    ...
    des états de polarisation. Cette méthode offre un cadre plus intégrable 
    pour incorporer des mesures faibles dans des technologies quantiques. Dans la suite 
    ...
    \textcolor{red}{limites. Nous} présenterons ensuite une méthode innovante, 
